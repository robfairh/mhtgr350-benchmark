...
def plot_axial_collapse(data, name, fig, V=1, dire='Z'):
    """
    Plots axial flux.
    Parameters:
    -----------
    ...
    lim = [2, 4, 5, 8, 9, 10, 13, 14, 16, 18, 24, 26]  # 12G
    G = len(lim)
...
def plot_radial_collapse(data, name, fig, piH=1):
    """
    Plots flux from curvilinear detector.
    Parameters:
    -----------
...
def plot_fullcore_collapse():
    ...
    V = Ah * (160 + 793 + 120)
    plot_axial_collapse(data, 'Axial1', fig, V)
    plot_axial_collapse(data, 'Axial3', fig, V)

    # Plots radial flux
    H = 793
    p = np.pi/180 * 360  # = 360 deg
    plot_radial_collapse(data, 'Radial1', fig, p*H)
    H = 79.3
    p = np.pi/180 * 2  # = 2 deg
    plot_radial_collapse(data, 'Radial2', fig, p*H)


def plot_fullcore12G():
    '''
    Plots the output of the detectors of the full core model.
    - Spectrum
    - 3 axial flux detectors
    ...
    - Pin power distribution
    '''
    data = st.read('oecd-fullcore12G_det1b1.m', reader='det')
    fig = 'fullcore12G'
